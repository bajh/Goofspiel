...
RoomManager.prototype.removeClientFromQueue = function(socket) {
  clientIndex = this.findClientIndex(socket);
  if (clientIndex != null) {
  //Socket.io deletes rooms that have no connections, so we only have to remove the client from the queue
    this.unmatchedSockets.splice(unmatchedSockets[clientIndex],1);
  }
}

RoomManager.prototype.findClientIndex = function(socket) {
  clientIndex = null;
  for (var i = 0; i < unmatchedSockets.length; i++) {
